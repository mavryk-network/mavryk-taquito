...
import { MichelsonMap, ViewSimulationError } from '@mavrykdynamics/webmavryk';
import { stringToBytes, InvalidUriError } from '@mavrykdynamics/webmavryk-tzip16';
...
describe('Tzip12 contract abstraction test', () => {
  ...
  it('Test 1 for getContractMetadata(): Should return the `Tzip-016` contract metadata', async () => {
    mockTzip16ContractAbstraction.getMetadata.mockResolvedValue({
      uri: 'https://test',
      metadata: { name: 'WebMavryk test' },
    });

    expect(await tzip12Abs['getContractMetadata']()).toEqual({ name: 'WebMavryk test' });
  });
  ...
  it('Test 1 for executeTokenMetadataView(): Should properly return the TokenMetadata', async () => {
    ...
    tokenMap.set('name', stringToBytes('WebMavryk'));
