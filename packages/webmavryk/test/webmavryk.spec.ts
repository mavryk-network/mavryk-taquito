...
import { MavrykToolkit, SetProviderOptions, Wallet, RpcPacker } from '../src/webmavryk';
...
import { RpcInjector } from '../src/webmavryk';
...
describe('MavrykToolkit test', () => {
  ...
  providerKey.forEach((key) => {
    it(`setting ${key} provider should not override the forger provider`, () => {
      expect(toolkit['_context'].forger).toBeInstanceOf(WebMavrykLocalForger);
