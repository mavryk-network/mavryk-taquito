...
describe('DefaultGlobalConstantsProvider tests', () => {
  ...
  it('register two global constants at the same time and retrieve their value by their hash', async () => {
    ...
    expect(michelineExpr2).toEqual(
      await defaultGlobalConstantsProvider.getGlobalConstantByHash(hash2)
    );
  });
  ...
  it('throws an error when querying a global constant that is not registered', async () => {
    ...
    try {
      ...
      expect(err.message).toEqual(
        'Please load the value associated with the constant expru5X5fvCer8tbRkSAtwyVCs9FUCq46JQG7QCAkhZSumjbZBUGzb using the loadGlobalConstant method of the DefaultGlobalConstantsProvider.'
      );
    }
  });
});
