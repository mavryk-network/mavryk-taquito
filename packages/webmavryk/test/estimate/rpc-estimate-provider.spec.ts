...
import { OpKind, PvmKind } from '@mavrykdynamics/webmavryk-rpc';
import { TransferTicketParams } from '../../src/operations/types';
import { InvalidAddressError } from '@mavrykdynamics/webmavryk-utils';
  ...
  describe('batch', () => {
    ...
    it('should produce a batch operation, with reveal', async () => {
      ...
      expect(estimate[0]).toMatchObject({
        gasLimit: 1000,
        storageLimit: 0,
        suggestedFeeMumav: 382,
      });
      expect(estimate[1]).toMatchObject({
        gasLimit: 1330,
        storageLimit: 93,
        suggestedFeeMumav: 350,
      });
      expect(estimate[2].suggestedFeeMumav).toEqual(317);
      expect(estimate[3].suggestedFeeMumav).toEqual(317);
      expect(estimate[2]).toMatchObject({
        gasLimit: 1000,
        storageLimit: 0,
        suggestedFeeMumav: 317,
      });
...
describe('RPCEstimateProvider test wallet', () => {
  ...
  describe('updateConsensusKey', () => {
    it('should return estimate for updateConsensusKey operation', async () => {
      ...
      expect(estimate.suggestedFeeMumav).toEqual(297);
    });
  });
  ...
  describe('smartRollupAddMessages', () => {
    it('should return the correct estimate for smartRollupAddMessages op', async () => {
      ...
      expect(estimate.storageLimit).toEqual(0);
      expect(estimate.suggestedFeeMumav).toEqual(298);
    });
  ...
  describe('smartRollupOriginate', () => {
    it('Should return the correct estimate for SmartRollupOriginate operation', async () => {
      ...
      expect(estimate.suggestedFeeMumav).toEqual(734);

      expect(estimate).toMatchObject({
        gasLimit: 3849,
        storageLimit: 6572,
        suggestedFeeMumav: 734,
        minimalFeeMumav: 714,
      });
    });
  });
