...
If the hash is present. Compute the sha256 hash of the JSON string before returning to webmavryk clients.
...
Consider adding a request interceptor in WebMavryk to allow user specify custom request authentication mecanism. In general this is an improvement that will benefit every component of WebMavryk that use http (RpcClient for example)
...
Consideration:
Data is encoded in byte in this protocol. Use the function `bytesToString` from `tzip16-utils.ts` to decode it to a string. Then use JSON.parse on the string to produce a javascript object useable by WebMavryk users.
