...
CONFIGS().forEach(({ rpc, protocol, setup, lib }) => {
  ...
  describe(`Test forging pseudo entrypoints: ${rpc}`, () => {
    ...
    // for every new protocol need to check https://protocol.mavryk.org/shell/p2p_api.html#alpha-entrypoint-determined-from-data-8-bit-tag for the latest entrypoints and corresponding tag
    let boreasEntrypoint = { 0: 'default', 1: 'root', 2: 'do', 3: 'set_delegate', 4: 'remove_delegate', 5: 'deposit', 6: 'stake', 7: 'unstake', 8: 'finalize_unstake', 9: 'set_delegate_parameters' }
    ...
    Object.values(boreasEntrypoint).forEach(name => {
