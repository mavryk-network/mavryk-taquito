...
A comparison of the methods implemented by Web3js and Webmavryk can put in perspective their similarities and differences.
...
Web3js comes as a single package containing all the necessary tools to connect to the Ethereum blockchain, while Webmavryk is made of various packages that you can install and import whenever you need them.
...
Webmavryk also exports a class named `MavrykToolkit` that needs to be instantiated before being used, similar to the Web3js code.
...
Both Web3js and Webmavryk use the `getBalance` method with the account address to fetch its spendable balance from the network.
...
The two approaches are radically different here: Web3js constructs the contract interface from the "ABI" that's shipped with the dapp while Webmavryk fetches the contract structure directly from the blockchain to construct the interface.
