 ...
 **boreasnet**: `npm run test:boreasnet rpc-nodes.spec.ts`
...
```
  Test calling all methods from RPC node: http://boreasnet.i.ecadinfra.com:8732/
    ...
    ✓ Verify that rpcClient.getProtocols will list past and present Mavryk protocols (32 ms)
...
```
  Test calling all methods from RPC node: http://localhost:20000
    ...
    ✓ Verify that rpcClient.getProtocols will list past and present Mavryk protocols (6 ms)
