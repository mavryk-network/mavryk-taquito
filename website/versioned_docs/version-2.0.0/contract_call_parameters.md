...
> You can find the tests used to check these values [in this GitHub repo](https://github.com/claudebarde/webmavryk-contract-call-params)
...
| option string            | Some "Mavryk"           | "Mavryk"             | "Mavryk" or {Some: "Mavryk"}                       |
| option (list nat)        | Some { 6 ; 7 ; 8 ; 9 } | [6, 7, 8, 9]        | [6, 7, 8, 9] or {Some: [6, 7, 8, 9]}             |
| option (pair string nat) | Some (Pair "Mavryk" 8)  | "Mavryk", 8          | {0: "Mavryk", 1: 8} or {Some: {0: "Mavryk", 1: 5}} |
...
| pair (pair int nat) (pair string mumav)                                | Pair (Pair 6 7) (Pair "Mavryk" 500000)                                | 6, 7, "Mavryk", 50_0000 | { 0: 6, 1: 7, 2: "Mavryk", 3: 50_000 }                    |
| pair (pair (int %one) (nat %two)) (pair (string %three) (mumav %four)) | Pair (Pair (6 %one) (7 %two)) (Pair ("Mavryk" %three) (500000 %four)) | 6, 7, "Mavryk", 50_0000 | { "one": 6, "two": 7, "three": "Mavryk", "four": 50_000 } |
...
```ts
let opWithWallet = await mavryk.wallet.transfer({
  to: 'KT1...',
  amount: 0,
  parameter: {
    entrypoint: 'default',
...
```ts
const opWithSigner = await mavryk.contract.transfer({
  to: 'KT1...',
  amount: 0,
  parameter: {
    entrypoint: 'default',
