...
The operation is forged like any other operation you would create with Webmavryk. For example, if you wish to create a contract call:
```typescript
...
- the function to run when a confirmation is received with a parameter holding [different properties](https://webmavryk.mavryk.org/typedoc/classes/_webmavryk_webmavryk.walletoperation.html#confirmationobservable), for example, details about the current block or the confirmation status
